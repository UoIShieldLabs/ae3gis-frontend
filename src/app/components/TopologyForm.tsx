...
interface TopologyFormProps {
  ...
  onSave: (data: CreateTopologyRequest) => Promise<Topology | null>;
  onCancel?: () => void;
}
...
const DEFAULT_NODE: TopologyNode = {
  name: "",
  template_key: "",
  x: 0,
  y: 0,
...
export default function TopologyForm({
  initialTopology,
  onSave,
  onCancel,
}: TopologyFormProps) {
  ...
  // Form state - use settings default if no initial topology
  const [name, setName] = useState(initialTopology?.name || "");
  const [description, setDescription] = useState(initialTopology?.description || "");
  const [projectName, setProjectName] = useState(
    initialTopology?.definition.project_name || settings.defaultProjectName || ""
  );

  // Nodes by layer
  const [itNodes, setItNodes] = useState<TopologyNode[]>([]);
  const [dmzNodes, setDmzNodes] = useState<TopologyNode[]>([]);
  const [otNodes, setOtNodes] = useState<TopologyNode[]>([]);
  ...
  useEffect(() => {
    if (!initialTopology && !projectName && settings.defaultProjectName) {
      setProjectName(settings.defaultProjectName);
    }
  }, [settings.defaultProjectName, initialTopology, projectName]);

  // Initialize from existing topology
  useEffect(() => {
    if (initialTopology) {
      const def = initialTopology.definition;
      setName(initialTopology.name);
      setDescription(initialTopology.description || "");
      ...
      const it: TopologyNode[] = [];
      const dmz: TopologyNode[] = [];
      const ot: TopologyNode[] = [];
  ...
  const updateNode = (layer: LayerType, index: number, node: TopologyNode) => {
    if (layer === "IT") {
  ...
  const buildTemplateMap = (allNodes: TopologyNode[]): Record<string, string> => {
    const map: Record<string, string> = {};
    
    const processNode = (node: TopologyNode) => {
  ...
  const handleSave = async () => {
    ...
    // Validation
    if (!name.trim()) {
      setError("Topology name is required");
      return;
    }
    if (!projectName.trim()) {
    ...
    try {
      ...
      // Create request
      const request: CreateTopologyRequest = {
        name,
      ...
      if (!result) {
        setError("Failed to save topology");
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-lg p-4">
        <h2 className="text-lg font-semibold mb-4">Topology Details</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g., DHCP Lab Topology"
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        >
          ...
          {initialTopology ? "Update Topology" : "Save Topology"}
        </button>
      </div>
    </div>
  );
